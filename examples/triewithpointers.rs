use trie::trie::{Trie, Prefix, PrefixAs, NoMeta};
...
fn main() {
    ...
    let spfx = Prefix4NoMeta::new(0b0100_0001_0000_0000_0000_0000_1111_1111_u32, 32);
    println!("search for: {:?}", spfx);

    let s_spfx= trie.match_longest_prefix(&spfx);
    println!("lmp: {:?}", s_spfx);

    let spfx = Prefix4NoMeta::new(std::net::Ipv4Addr::new(64, 10, 10, 10).into(), 32);
    println!("search for: {:?}", spfx);

    let s_spfx = trie.match_longest_prefix(&spfx);
    println!("lmp: {:?}", s_spfx);

    let spfx = Prefix4NoMeta::new(std::net::Ipv4Addr::new(13, 10, 10, 10).into(), 24);
    println!("search for: {:?}", spfx);
