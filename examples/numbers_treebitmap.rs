...
fn main() {
    ...
    for strides in strides_vec.iter().enumerate() {
        ...
        let total_prefixes = tree_bitmap.stats.iter().fold(0, |mut acc, c| {
            acc += c.prefixes_num.iter().fold(0, |mut sum, l| {
                sum += l.count;
                sum
            });
            acc
        ...
        println!(
            "\"node_size_b\": {},",
            std::mem::size_of::<trie::treebitmap::SizedStrideNode<u32, PrefixAs>>()
        );
        println!(
            "\"nodes_mem_kb\": {},",
        ...
        println!(
            "\"prefixes_mem_kb\": {:?},",
            total_prefixes * std::mem::size_of::<Prefix<u32, PrefixAs>>() / 1024
        );
        println!(
            "\"prefixes_per_node\": {},",
