...
mod test {
    ...
    fn test_tree_ipv4() {
        ...
        let res = trie
            .match_longest_prefix(&Prefix::<u32, NoMeta>::new(
    ...
    #[test]
    fn multiple_insert() {
        let mut trie: RadixTrie<u32, NoMeta> = RadixTrie::new();
        let pfx1 = Prefix::<u32, NoMeta>::new(std::net::Ipv4Addr::new(192, 1, 12, 0).into(), 24);
        let pfx2 = Prefix::<u32, NoMeta>::new(std::net::Ipv4Addr::new(192, 1, 12, 0).into(), 24);
        trie.insert(&pfx1);
        trie.insert(&pfx2);

        assert_eq!(trie.match_longest_prefix(&pfx1).unwrap(), &pfx1);
        assert_eq!(trie.match_longest_prefix(&pfx1).unwrap(), &pfx2);
        assert_eq!(trie.match_longest_prefix(&pfx2).unwrap(), &pfx1);
        assert_eq!(trie.match_longest_prefix(&pfx2).unwrap(), &pfx2);
    }

    #[test]
    fn test_cascade_ipv4() {
        let mut trie: RadixTrie<u32, NoMeta> = RadixTrie::new();

        let mut pfx_vec: Vec<Prefix<u32, NoMeta>> = (0..32)
            .map(|i_len| {
                Prefix::<u32, NoMeta>::new(std::net::Ipv4Addr::new(1, 0, 0, 0).into(), i_len)
            })
            .collect();
        ...
        for s_len in 2..32 {
            let s_pfx =
                Prefix::<u32, NoMeta>::new(std::net::Ipv4Addr::new(1, 0, 0, 0).into(), s_len);
            let res = trie.match_longest_prefix(&s_pfx);
            println!("{:?}", s_pfx);

            assert_eq!(res.unwrap(), &s_pfx);
        }
    }
